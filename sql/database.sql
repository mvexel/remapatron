...
BEGIN
	cmd := 'SELECT 
		(SELECT "count" FROM remapathonresults ORDER BY tstamp DESC LIMIT 1) - 
		(SELECT "count" FROM remapathonresults WHERE tstamp < NOW() - INTERVAL ''' 
		|| in_interval || 
...
--
-- Name: mr_donesince(integer, integer); Type: FUNCTION; Schema: public; Owner: osm
--

CREATE FUNCTION mr_donesince(in_interval integer, chid integer) RETURNS integer
    LANGUAGE plpgsql
    AS $$
  DECLARE 
	cmd text;
...
BEGIN
	cmd := 'SELECT 
		(SELECT "count" FROM remapathonresults WHERE challengeid = ' || chid || ' ORDER BY tstamp DESC LIMIT 1) - 
		(SELECT "count" FROM remapathonresults WHERE challengeid = ' || chid || ' AND tstamp < NOW() - INTERVAL ''' 
		|| in_interval || 
		'HOUR'' ORDER BY tstamp DESC LIMIT 1);';
	EXECUTE cmd INTO result;
	RETURN -result;
...
ALTER FUNCTION public.mr_donesince(in_interval integer, chid integer) OWNER TO osm;
...
BEGIN
  RETURN QUERY (
  WITH initialq AS ( 
    SELECT osmid, geom FROM mr_nolanechallenge
    WHERE fixflag < 3 
-- AND "type" IN ('motorway','motorway_link','trunk','trunk_link') 
AND (EXTRACT(EPOCH FROM now() - COALESCE(locked, (now() + interval '1 week'))) / 3600) < 0 
    LIMIT 1) 
  -- FIXME make challengeid a parameter
  SELECT challenge.osmid, ST_AsGeoJSON(challenge.geom) FROM mr_nolanechallenge challenge, initialq
  WHERE initialq.geom <#> challenge.geom < 0.01
  AND fixflag < 3
  -- AND "type" IN ('motorway','motorway_link','trunk','trunk_link') 
  AND (EXTRACT(EPOCH FROM now() - COALESCE(locked, (now() + interval '1 week'))) / 3600) < 0
  AND challenge.osmid != initialq.osmid
  LIMIT 25);
  RETURN;
END;
$$;
...
--
-- Name: mr_upsert(integer, bigint, integer); Type: FUNCTION; Schema: public; Owner: osm
--

CREATE FUNCTION mr_upsert(in_fixflag integer, in_osmid bigint, in_chid integer) RETURNS void
    LANGUAGE plpgsql
    AS $$ 
DECLARE 
BEGIN 
    UPDATE maproulette SET fixflag = in_fixflag WHERE challengeid = in_chid AND osmid = in_osmid; 
    IF NOT FOUND THEN 
    INSERT INTO maproulette values (in_osmid, in_fixflag, in_chid); 
    END IF; 
END; 
$$;


ALTER FUNCTION public.mr_upsert(in_fixflag integer, in_osmid bigint, in_chid integer) OWNER TO osm;
...
BEGIN 
  ...
  LOOP
    UPDATE maproulette SET fixflag = in_fixflag WHERE osmid = in_osmid[i] AND challengeid = in_chid; 
    IF NOT FOUND THEN 
      INSERT INTO maproulette (osmid, fixflag) values (in_osmid[i], in_fixflag, in_chid); 
    END IF; 
  END LOOP;
END; 
$$;
...
CREATE VIEW mr_currentchallenge AS
    SELECT a.deadend_node_id AS osmid, a.the_geom AS geom, a.deadend_way_id AS osmid_way, a.deadend_geom AS geom_way, a.distance_min, a.closeest_way_id AS secondary_osmid, a.closest_way_geom AS secondary_geom, COALESCE(b.fixflag, 0) AS fixflag FROM (tnav_connectivity_errors a LEFT JOIN maproulette b ON ((a.deadend_way_id = b.osmid)));
...
--
-- Name: mr_nolanechallenge; Type: VIEW; Schema: public; Owner: osm
--

CREATE VIEW mr_nolanechallenge AS
    SELECT a.id AS osmid, a.type, a.linestring AS geom, b.locked, COALESCE(b.challengeid, 2) AS challengeid, COALESCE(b.fixflag, 0) AS fixflag FROM (tnav_ways_no_lanes a LEFT JOIN maproulette b ON ((a.id = b.osmid)));
